...
import HaloPSAClient from 'halopsa-client';
...
export const handler = AWSXRay.captureAsyncFunc('CallLoggingHandler', async (event, context) => {
  const startTime = Date.now();
  let metric = { success: false };

  try {
    // Extract call attributes from Connect event payload
    // (This Lambda should be wired as a Contact Flow function)
    ...
    if (!customerId || !phoneNumber || !agentId) {
      console.warn('Missing required data for CallLogging');
...
function parseEvent(event) {
  ...
  // your Contact Flow should pass attributes: CustomerId, PhoneNumber, AgentId, StartTimestamp, EndTimestamp, Transcript
  // Example:
  // {
  //   Details: {
  //     ContactData: {
  ...
  //         AgentId: 'AGENT-001',
  //         StartTimestamp: '2025-06-01T12:34:56Z',
  //         EndTimestamp: '2025-06-01T12:45:00Z',
